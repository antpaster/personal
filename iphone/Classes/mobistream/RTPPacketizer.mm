...
static void tonet_longlong(uint8_t* p, long long l)
{
    p[0] = (l >> 56) & 0xff;
    p[1] = (l >> 48) & 0xff;
    p[2] = (l >> 40) & 0xff;
...
- (NSMutableArray *) onVideoData1:(NSArray*) data time:(double) pts
{
...
- (NSMutableArray *) onAudioData1:(NSArray*) data time:(double) pts
{
    NSMutableArray *packetArray = [[NSMutableArray alloc] init];
...
- (NSMutableArray *) onVideoData:(NSArray*) data time:(double) pts
{
    ...
    for (int i = 0; i < nNALUs; i++){
        ...
        if (cBytes < max_single_packet){
            [self writeHeaderP:packet marker:bLast time:pts];
            memcpy(packet + rtp_header_size, [nalu bytes], cBytes);

            //[self sendPacket:packet length:(cBytes + rtp_header_size)];
            
            NSMutableData *data = [NSMutableData dataWithBytes:packet length:(cBytes + rtp_header_size)];
            
            ///////////////////
            uint8_t tsArr[9];
            memset(&tsArr[0], 0, 9);
            long ts = [self currentTimeMillis];
            tonet_longlong(&tsArr[0], ts);
            tsArr[8] = 9;
            ...
            [data appendBytes:tsArr length:9];

            //        NSData *tsData = [NSData dataWithBytes:tsArr length:9];
            //        NSLog(@"%ld", ts);
            //        NSLog(@"%@", [self hexadecimalString:tsData]);
            /////////////////////
            
            [packetArray addObject:data];
            _packets++;
            _bytesSent += (cBytes + rtp_header_size);
        }else{
            unsigned char NALU_Header = pSource[0];
            ...
            while (cBytes){
                int cThis = (cBytes < max_fragment_packet)? cBytes : max_fragment_packet;
                BOOL bEnd = (cThis == cBytes);
                [self writeHeaderP:packet marker:(bLast && bEnd) time:pts];
                unsigned char* pDest = packet + rtp_header_size;
                ...
                NSMutableData *data = [NSMutableData dataWithBytes:packet length:(int)(pDest - packet)];
...
//    [packetArray addObject: [NSNumber numberWithLong:packetTs]];
    return packetArray;
}

- (void) writeHeaderP:(uint8_t*) packet marker:(BOOL) bMarker time:(double) pts
{
//    packet[0] = 0x80;   // v= 2
    packet[0] = 0xA0;   // v= 2
    if (bMarker){
...
- (void) writeAudioHeaderP:(uint8_t*) packet marker:(BOOL) bMarker time:(double) pts size:(int)size
{
    packet[0] = 0xA0;   // v= 2
    if (bMarker){
        packet[1] = 97 | 0x80;
    ...
    pts -= _ptsBase;
    audioSize = audioSize + 1024;
    uint64_t rtp = (uint64_t)(audioSize);
...
- (NSMutableArray *) onAudioData:(NSArray*) data time:(double) pts
{
    ...
    for (int i = 0; i < nNALUs; i++){
        ...
        BOOL bLast = (i == nNALUs-1);
        const unsigned char* pSource = (unsigned char*)[nalu bytes];
        [self writeAudioHeaderP:packet marker:bLast time:pts size:cBytes - 4];
        memcpy(packet + rtp_header_size, [nalu bytes], cBytes);
